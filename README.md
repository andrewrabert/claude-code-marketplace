...
| `llms`       | Use any time llms.txt or "llms" is referenced, or when working with a project that publishes one (pydantic, pydantic-settings, "Model Context Protocol specification", "mcp specification", fastmcp) — fetch that project's canonical remote llms.txt as the source of truth for current docs. Covers what llms.txt is (an index of pages to fetch) and how to follow it. |
