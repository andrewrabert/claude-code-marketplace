...
description: Use when reviewing an implementation plan or design doc for how well it absorbs change — DRY vs YAGNI balance, open/closed extension points, testability seams, and reversibility of decisions.
---

# Change Resilience

**Code is read and changed far more than it's written. A good plan makes the likely changes cheap without paying upfront for changes that may never come. The two failure modes pull opposite ways — too much speculative abstraction, or too much copy-paste — and a reviewer has to call both.** Judge how the design ages, not how it looks on day one.
...
- **Testability seams** — can the units be tested without spinning up the whole world? Are dependencies injectable (passed in) rather than reached for (globals, singletons, direct construction of heavy collaborators)? A plan with no seam for the network/clock/filesystem/GPU is a plan for untestable code. This often *is* the separation-of-concerns check from the test's point of view.
- **Verification tests the invariant, not just asserts it** — when a plan rests on an equivalence or correctness invariant ("the GPU output is identical to the CPU path," "the new encoder round-trips the old format"), does its *own verification section* actually test that invariant, or only state it and check by eyeball? A plan whose verification is all-manual-runtime ("open it and confirm it looks the same") has no guard against the very drift its DRY findings warn about — the shared seam exists precisely so an automated equivalence/golden check can pin it. An asserted invariant with no test is a regression waiting for the first refactor.
...
- A `switch`/`match` on a type tag that the plan admits will keep growing — and lives in several files.
